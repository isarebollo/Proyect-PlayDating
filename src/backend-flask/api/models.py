    ...
    def participantes(self):
        ...
        for participante_evento in participantes_evento:
            ...
            participante = {"nombre": nombre,
                            "cantidad": cantidad, "id": participante_id}
...
class Favorito(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    usuario_inicial_id = db.Column(
        db.Integer, db.ForeignKey('usuario.id'), nullable=False)
    usuario_favorito_id = db.Column(
        db.Integer, db.ForeignKey('usuario.id'), nullable=False)
    usuario_inicial = db.relationship(
        'Usuario', foreign_keys=[usuario_inicial_id])
    usuario_favorito = db.relationship(
        'Usuario', foreign_keys=[usuario_favorito_id])
...
class Invitacion(db.Model):
    ...
    usuario_creador_id = db.Column(
        db.Integer, db.ForeignKey('usuario.id'), nullable=False)
    usuario_invitado_id = db.Column(
        db.Integer, db.ForeignKey('usuario.id'), nullable=False)
    evento_id = db.Column(db.Integer, db.ForeignKey(
        'evento.id'), nullable=False)
    evento = db.relationship('Evento')
    usuario_creador = db.relationship(
        'Usuario', foreign_keys=[usuario_creador_id])
    usuario_invitado = db.relationship(
        'Usuario', foreign_keys=[usuario_invitado_id])
